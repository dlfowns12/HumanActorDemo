...
namespace NatSuite.Recorders {
    ...
    public sealed class MP4Recorder : IMediaRecorder {
        ...
        public MP4Recorder(string filePath, int width, int height, float frameRate, int sampleRate = 0, int channelCount = 0, int bitrate = (int)(960 * 540 * 11.4f), int keyframeInterval = 3) => this.recorder = new NativeRecorder((callback, context) => Bridge.CreateMP4Recorder(width, height, frameRate, bitrate, keyframeInterval, sampleRate, channelCount, filePath));
#else
